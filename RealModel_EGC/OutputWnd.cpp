...
// 2011.11.23
void COutputWnd::AddStrToOutputBuild(CString& strAddToList)  
{  
    m_wndOutputBuild.AddString(strAddToList); //作为演示，暂时对一项进行操作 
	int nCount = m_wndOutputBuild.GetCount(); // 获取 string 数
	m_wndOutputBuild.SetTopIndex(nCount - 1); // 显示 最后 一行
}
...
{  
	...
	int nCount = m_wndOutputDebug.GetCount(); // 获取 string 数
	m_wndOutputDebug.SetTopIndex(nCount - 1); // 显示 最后 一行
}

// 2011.11.23
void COutputWnd::AddStrToOutputFind(CString& strAddToList)  
{  
    m_wndOutputFind.AddString(strAddToList); //作为演示，暂时对一项进行操作 
	int nCount = m_wndOutputFind.GetCount(); // 获取 string 数
	m_wndOutputFind.SetTopIndex(nCount - 1); // 显示 最后 一行
}